  ...
  private async checkAndRenewTokens() {
    ...
    for (const integration of integrations) {
      try {
        ...
        this.logger.error(
          `Error renewing token for ${integration.insurerName}:`,
          error
        );
      }
    }
  ...
  private async renewToken(integration: any) {
    try {
      ...
      if (!integration.refreshToken) {
        this.logger.warn(
          `No refresh token available for ${integration.insurerName}`
        );
        return;
      }
      ...
      this.logger.log(
        `Successfully renewed token for ${integration.insurerName}`
      );
    } catch (error) {
      this.logger.error(
        `Failed to renew token for ${integration.insurerName}:`,
        error
      );
      throw error;
    }
