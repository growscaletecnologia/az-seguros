...
export class CacheService {
  ...
  async set(key: string, value: any, ttl: number = this.TTL): Promise<void> {
    try {
      ...
      await this.redis.multi().set(key, JSON.stringify(cacheValue)).expire(key, ttl).exec()
  ...
  generateQuoteKey(params: {
    ...
    const { destinyGroup, paxCount, ageGroup, days, insurerId } = params
    const key = `QUOTE:${destinyGroup}:${paxCount}:${ageGroup}:${days}:${insurerId}`
