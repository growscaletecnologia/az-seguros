...
export const postsService = {
	 ...
	 */
	async getPublishedPosts(page = 1, limit = 10, search?: string) {
	...
	async getPostsByStatus(
		...
		const { data } = await api.get(
			`/posts/status/${status}?${params.toString()}`,
		);
		return data;
	},
	...
	async uploadCoverImage(postId: string, file: File) {
		...
		const { data } = await api.post(
			`/posts/${postId}/upload-cover-image`,
			formData,
			{
				headers: {
	...
	async uploadMainImage(postId: string, file: File) {
		...
		const { data } = await api.post(
			`/posts/${postId}/upload-main-image`,
			formData,
			{
				headers: {
					"Content-Type": "multipart/form-data",
