	...
	const onSubmit = async (values: FrontSectionFormValues) => {
		try {
			...
			if (editingSection) {
				...
				savedSection = await frontSectionsService.update(
					editingSection.id,
					sectionData,
				);
												...
												<SelectTrigger>
													<SelectValue placeholder="Selecione uma cor">
														{field.value && (
															<div className="flex items-center gap-2">
																<div
																	className={`w-4 h-4 rounded bg-${field.value}-500`}
																></div>
											...
											<SelectContent>
												{AVAILABLE_COLORS.map((color) => (
													<SelectItem key={color} value={color}>
														<div className="flex items-center gap-2">
															<div
																className={`w-4 h-4 rounded bg-${color}-500`}
															></div>
