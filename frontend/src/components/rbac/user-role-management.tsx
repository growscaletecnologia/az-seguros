 ...
 */
export function UserRoleManagement({ userId, userRoles, onRolesUpdated }: UserRoleManagementProps) {
      ...
      <div className="flex flex-wrap gap-2 min-h-10">
        {userRoles.length === 0 ? (
          ...
          userRoles.map(userRole => {
            const role = roles.find(r => r.id === userRole.roleId) || userRole.role
            return (
              <Badge key={userRole.roleId} variant="secondary" className="flex items-center gap-1">
                ...
                <Button
                  ...
                  disabled={removing || (role?.isSystem && userRoles.length === 1)}
                  title="Remover papel"
                >
                  <X className="h-3 w-3" />
                </Button>
