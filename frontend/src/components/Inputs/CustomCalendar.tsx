			...
			<Popover>
				<PopoverTrigger asChild>
					<div className="flex flex-row gap-3">
						...
						>
							...
							{range?.from ? (
								<div className="text-[16px]">{format(range.from, "dd/MM/yyyy", { locale: ptBR })}</div>
							) : (
								<span className="text-white/90 text-[16px] font-semibold">Embarque no Brasil</span>
							)}
						</div>
						<div
							className={cn(
						...
						>
							...
							{range?.to ? (
								<div className="text-[16px]">{format(range.to, "dd/MM/yyyy", { locale: ptBR })}</div>
							) : (
								<span className="text-white/90 text-[16px] font-semibold">Desembarque no Brasil</span>
							)}
						</div>
					</div>
				</PopoverTrigger>
				...
				<PopoverContent className="w-auto p-0">
					...
					{/* <div className="flex items-center justify-between border-t p-3 text-sm">
						<button className="underline" onClick={() => onChange(undefined)}>
							Limpar
						</button>
						<div>
							{nights > 0
