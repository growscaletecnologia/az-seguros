...
export default function GtmSettingsForm({
	...
	const loadGtmSettings = async () => {
		try {
			...
			const extractedId = extractGtmId(
				settings.gtm_head_code || settings.gtm_body_code,
			);
	...
	const saveGtmSettings = async () => {
		try {
			...
			const extractedId = extractGtmId(
				gtmSettings.gtm_head_code || gtmSettings.gtm_body_code,
			);
			setGtmId(extractedId);
			...
			<CardContent className="space-y-4">
				...
				<div className="space-y-2">
					...
					<Textarea
						...
						onChange={(e) =>
							setGtmSettings({ ...gtmSettings, gtm_head_code: e.target.value })
						}
            ...
            Cole o código do GTM que deve ser inserido na seção &lt;head&gt; do site.
				...
				<div className="space-y-2">
					...
					<Textarea
						...
						onChange={(e) =>
							setGtmSettings({ ...gtmSettings, gtm_body_code: e.target.value })
						}
						className="font-mono text-sm"
					/>
					{/* <p className="text-xs text-muted-foreground">
            Cole o código do GTM que deve ser inserido logo após a abertura da tag &lt;body&gt;.
				...
				{gtmId && validationErrors.length === 0 && (
					<Alert>
						...
						<AlertDescription>
							Google Tag Manager configurado corretamente com ID:{" "}
							<strong>{gtmId}</strong>
						</AlertDescription>
					</Alert>
				)}
