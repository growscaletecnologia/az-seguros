...
import { mockProducts, getProductsForComparison } from '../../lib/mock-products';
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <th key={product.id} className="px-6 py-4 text-center relative">
                      <div className="space-y-2">
                        <div className="flex items-center justify-center space-x-2">
                          <h3 className="font-bold text-gray-900">{product.plano}</h3>
                        ...
                        <p className="text-sm text-gray-600">{product.seguradora}</p>
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                {mockProducts[0].detalhes.map((detail, index) => (
                  <tr key={detail.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    {productsToCompare.map((product) => {
                      const productDetail = product.detalhes.find(d => d.id === detail.id);
                      return (
        ...
        <div className="mt-8 bg-white rounded-lg shadow-sm p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Resumo da Comparação</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {productsToCompare.map((product) => (
              <div key={product.id} className="border rounded-lg p-4">
                <h3 className="font-semibold text-gray-900 mb-2">{product.plano}</h3>
                <p className="text-sm text-gray-600 mb-3">{product.seguradora}</p>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span>Cobertura Médica:</span>
                    <span className="font-medium">USD {product.coberturaMedica.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Bagagem:</span>
                    <span className="font-medium">USD {product.coberturaBagagem.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>COVID-19:</span>
                    <span className="font-medium">{product.coberturaPandemia ? 'SIM' : 'NÃO'}</span>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t">
                  <div className="text-center">
                    <p className="text-lg font-bold text-green-600">{formatPrice(product.preco)}</p>
