...
const mockPlans: Plan[] = [
  {
    ...
    beneficios: ["Cobertura COVID-19", "Telemedicina 24h", "Cancelamento de viagem", "Prática de esportes"],
  ...
  },
  {
    ...
    beneficios: ["Cobertura COVID-19", "Atendimento em português", "Cancelamento de viagem", "Regresso sanitário"],
  ...
  },
  {
    ...
    destaque: "Premium",
    beneficios: ["Cobertura COVID-19", "Telemedicina 24h", "Cancelamento de viagem", "Prática de esportes", "Cobertura odontológica"],
  ...
  },
  {
    ...
    destaque: "",
    beneficios: ["Cobertura COVID-19", "Telemedicina", "Cancelamento de viagem", "Prática de esportes", "Fisioterapia"],
    faixaEtaria: "Faixa etária: 0 a 75 anos",
    aumentoIdade: "R$ 252,48 maiores de 64 anos",
  },
];
...
const benefitsByPlan: Record<number, BenefitItem[]> = {
  1: [
    ...
    { titulo: "Despesa Médica Hospitalar Total", valor: "USD 60,000", extra: "Valor total de despesas médicas hospitalares." },
  ...
  3: [
    ...
    { titulo: "Validade Geográfica", valor: "EUROPA" },
    { titulo: "Franquia de DMH", valor: "NÃO" },
    { titulo: "Despesa Médica Hospitalar Total", valor: "USD 75.000", extra: "Inclui atendimento ambulatorial e hospitalar." },
    { titulo: "Telemedicina", valor: "SIM" },
...
function Checkbox({
  ...
  return (
    <label htmlFor={id} className="flex items-center gap-2 cursor-pointer select-none py-1.5">
      <input
        id={id}
        type="checkbox"
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-black/50">
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-blue-700 text-white px-5 py-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1 rounded hover:bg-blue-600/40">
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="px-6 py-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-6">
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Valor de Cobertura Médica</h4>
            <div>
          ...
          <div>
            <h4 className="font-semibold text-gray-900 mb-2">Valor de Cobertura COVID</h4>
          ...
          <div className="text-right">
            {plan.precoOriginal > plan.preco && (
              <p className="text-sm text-gray-500 line-through">
                {plan.precoOriginal.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
              </p>
            )}
            <p className="text-3xl font-extrabold text-emerald-700">
              {plan.preco.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
            </p>
        ...
        <div className="px-6 pb-6">
          ...
          <div className="overflow-hidden rounded-lg border">
            {(benefitsByPlan[plan.id] ?? []).map((row, i) => (
              <AccordionRow key={i} titulo={row.titulo} valor={row.valor} extra={row.extra} />
            ))}
          </div>
        </div>
      </div>
...
function AccordionRow({ titulo, valor, extra }: { titulo: string; valor?: string; extra?: string }) {
      ...
      >
        <div className="flex items-center gap-3">
          <span className="p-1 rounded-md bg-white shadow-sm">
            {open ? <Minus className="h-4 w-4 text-gray-600" /> : <Plus className="h-4 w-4 text-gray-600" />}
          </span>
          <span className="text-sm font-medium text-gray-800">{titulo}</span>
        </div>
        <div className="flex items-center gap-3">{valor && <span className="text-sm text-gray-800">{valor}</span>}</div>
      </button>
      {open && extra && (
...
export default function PlanosPage() {
  ...
  const options = useMemo(() => {
    const medicalMins = uniq(mockPlans.map((p) => p.coberturaMedica)).sort((a, b) => a - b);
    const faixaEtaria = uniq(mockPlans.map((p) => p.faixaEtaria || "").filter(Boolean)).sort();
  ...
  const filteredPlans = useMemo(() => {
    return mockPlans.filter((p) => {
      ...
      // Seguradoras
      if (filters.insurers.length && !filters.insurers.includes(p.seguradora)) return false;
      // Faixa etária (string)
      if (filters.faixaEtaria.length && !filters.faixaEtaria.includes(p.faixaEtaria || "")) return false;
      // Benefícios: exige TODOS os selecionados
      if (filters.benefits.length && !filters.benefits.every((b) => p.beneficios.includes(b))) return false;

      return true;
    });
  }, [filters]);

  const viewPlan = useMemo(() => mockPlans.find((p) => p.id === viewPlanId) || null, [viewPlanId]);

  const handleSelectPlan = (planId: number) => {
    setSelectedPlans((cur) =>
      cur.includes(planId) ? cur.filter((id) => id !== planId) : cur.length < 3 ? [...cur, planId] : cur
    );
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-2">Encontre o seguro de viagem perfeito para sua próxima aventura</p>
            </div>
         ...
         <div className="lg:col-span-2 lg:col-start-2">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-600">{filteredPlans.length} planos encontrados</span>
              </div>
                  ...
                  <div className="p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-1 mt-1">
                              <div className="flex items-center">
                                {[...Array(5)].map((_, i) => (
                                  <Star
                                    key={i}
                                    className={`h-4 w-4 ${
                                      i < Math.floor(plan.avaliacoes) ? "text-yellow-400 fill-current" : "text-gray-300"
                                    }`}
                                  />
                                ))}
                              </div>
                              ...
                              <div className="flex flex-col gap-0 relative bottom-2.5 left-8 right-0">
                                <span className="text-sm text-gray-600">{plan.faixaEtaria}</span>
                                <span className="text-sm text-gray-600">{plan.aumentoIdade}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="lg:w-64 text-center lg:text-right">
                        <div className="mb-4">
                          {plan.precoOriginal > plan.preco && (
                            <p className="text-sm text-gray-500 line-through">{formatPrice(plan.precoOriginal)}</p>
                          )}
                          <p className="text-3xl font-bold text-gray-900">{formatPrice(plan.preco)}</p>
              ...
              <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
                <div className="bg-white rounded-lg w-full max-w-7xl max-h-[92vh] overflow-hidden">
                  <div className="p-6 border-b flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <button onClick={() => setShowComparison(false)} className="text-sm text-emerald-700 hover:underline">
                        Voltar à pesquisa
                      </button>
                      <span className="text-gray-400">•</span>
                      <span className="text-sm text-gray-600">Compare os planos selecionados</span>
                    </div>
                  ...
                  <div className="px-6 pt-6 overflow-x-auto">
                    <div className={`grid gap-4`} style={{ gridTemplateColumns: `220px repeat(${getSelectedPlans().length}, minmax(260px,1fr))` }}>
                      <div className="min-w-[220px]">
                        <div className="border rounded-lg p-4">
                          <p className="text-sm font-medium text-gray-700 mb-2">Você pesquisou por:</p>
                      ...
                      {getSelectedPlans().map((plan) => (
                        <div key={plan.id} className="border rounded-lg">
                          <div className="p-4">
                            <h3 className="text-sm font-bold text-gray-900 leading-5">{plan.plano}</h3>
                            <p className="text-xs text-gray-600">{plan.seguradora}</p>
                            <div className="mt-3">
                              {plan.precoOriginal > plan.preco && (
                                <p className="text-xs text-gray-500 line-through">{formatPrice(plan.precoOriginal)}</p>
                              )}
                              <div className="flex items-baseline gap-2">
                                <p className="text-2xl font-extrabold text-emerald-700">{formatPrice(plan.preco)}</p>
                                <span className="text-[11px] px-2 py-0.5 bg-amber-100 text-amber-800 rounded">5% off no PIX</span>
                              </div>
                            </div>
                            <button
                  ...
                  <div className="px-6 pb-6 mt-6 overflow-auto">
                    {(() => {
                      ...
                      getSelectedPlans().forEach((p) => (benefitsByPlan[p.id] ?? []).forEach((b) => rowsSet.add(b.titulo)));
                      const rows = Array.from(rowsSet);
                      const getVal = (planId: number, titulo: string) => {
                        const found = (benefitsByPlan[planId] ?? []).find((b) => b.titulo === titulo);
                        return found?.valor ?? "—";
                      };
                      return (
                        <div className="border rounded-lg overflow-hidden">
                          <div className="bg-gray-50 px-4 py-3 border-b text-sm font-semibold text-gray-800">Coberturas Médicas</div>
                          <div className="divide-y">
                            {rows.map((titulo, idx) => (
                              <div
                                key={titulo}
                                className={`grid items-center text-sm ${idx % 2 === 0 ? "bg-white" : "bg-gray-50"}`}
                                style={{ gridTemplateColumns: `220px repeat(${getSelectedPlans().length}, minmax(260px,1fr))` }}
                              >
                                <div className="px-4 py-3 font-medium text-gray-800">{titulo}</div>
                                {getSelectedPlans().map((plan) => (
                                  <div key={plan.id + "-" + titulo} className="px-4 py-3 text-gray-900">
            ...
            {/* Modal de Detalhes */}
            {viewPlan && <PlanDetailsModal plan={viewPlan} onClose={() => setViewPlanId(null)} />}
          </div>
        </div>
      </div>
