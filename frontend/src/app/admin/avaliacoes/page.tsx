...
export default function AvaliacoesAdminPage() {
  ...
  const [selectedAvaliation, setSelectedAvaliation] = useState<Avaliation | null>(null);
  ...
  const currentItems = filteredAvaliacoes.slice(indexOfFirstItem, indexOfLastItem);
                  ...
                  {currentItems.length === 0 ? (
                    ...
                    currentItems.map((avaliation) => (
                      <TableRow key={avaliation.id}>
                        <TableCell className="font-medium">{avaliation.name}</TableCell>
                        <TableCell>
                          <div className="flex">{renderStars(avaliation.rating)}</div>
                        </TableCell>
                        <TableCell>{avaliation.location || '-'}</TableCell>
                        <TableCell>{formatDate(avaliation.createdAt)}</TableCell>
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            <Switch
                              checked={avaliation.active}
                              onCheckedChange={() => handleToggleActive(avaliation.id)}
                            />
                            <Badge variant={avaliation.active ? 'default' : 'outline'}>
              ...
              {filteredAvaliacoes.length > 0 && (
                <div className="flex items-center justify-between space-x-2 py-4">
                  <div className="text-sm text-gray-500">
                    Mostrando {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredAvaliacoes.length)} de {filteredAvaliacoes.length} avaliações
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button
                      variant="outline"
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {selectedAvaliation && (
            <div className="space-y-4">
              ...
              <div className="flex">{renderStars(selectedAvaliation.rating)}</div>
              ...
              <div className="text-sm text-gray-500">
                ...
                <p>Última atualização: {formatDate(selectedAvaliation.updatedAt)}</p>
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button
                  variant={selectedAvaliation.active ? 'destructive' : 'default'}
                  onClick={() => {
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input 
                ...
                onChange={(e) => setNewAvaliation({...newAvaliation, name: e.target.value})}
            ...
            <div className="grid gap-2">
              ...
              <Input 
                ...
                onChange={(e) => setNewAvaliation({...newAvaliation, rating: parseInt(e.target.value)})}
                required
              />
                ...
                {Array(5).fill(0).map((_, i) => (
                  <StarIcon
                    ...
                    className={cn(
                      ...
                      i < newAvaliation.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'
                    )}
                    onClick={() => setNewAvaliation({...newAvaliation, rating: i + 1})}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Textarea 
                ...
                onChange={(e) => setNewAvaliation({...newAvaliation, comment: e.target.value})}
            ...
            <div className="grid gap-2">
              ...
              <Input 
                ...
                onChange={(e) => setNewAvaliation({...newAvaliation, location: e.target.value})}
            ...
            <div className="flex items-center space-x-2">
              <Switch 
                ...
                onCheckedChange={(checked) => setNewAvaliation({...newAvaliation, active: checked})}
              />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateAvaliation}>
              Salvar
            </Button>
          </DialogFooter>
        </DialogContent>
