...
const CuponsPage = () => {
	...
	const toggleCupom = (id: number) => {
		setCupons(cupons.map((c) => (c.id === id ? { ...c, ativo: !c.ativo } : c)));
	};
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								setNovoCupom({ ...novoCupom, desconto: Number(e.target.value) })
							}
							min="0"
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								setNovoCupom({ ...novoCupom, dataExpiracao: e.target.value })
							}
						/>
					</div>
							...
							{cupons.map((cupom) => (
								<tr key={cupom.id} className="border-t">
									...
									<td className="px-4 py-2">
										{cupom.desconto}
										{cupom.tipoDesconto === "percentual" ? "%" : " R$"}
									</td>
									...
									<td className="px-4 py-2">
										...
										<div className="w-full bg-gray-200 rounded-full h-2 mt-1">
											<div
												...
												style={{
													width: `${(cupom.usosAtuais / cupom.limitesUso) * 100}%`,
												}}
											></div>
										</div>
									</td>
