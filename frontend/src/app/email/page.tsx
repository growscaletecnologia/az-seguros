...
type Order = {
  ...
  viagem: { origem: string; destino: string; embarque: string; desembarque: string };
  produto: {
    seguradora: string;
    plano: string;
    ...
    <div className="w-full max-w-[760px] rounded-xl border shadow-sm overflow-hidden bg-white">
      ...
      <div className="px-6 pb-6">
        ...
        <div className="mt-4 mb-2">
          <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${badgeClass}`}>
        ...
        <div className="mt-4 rounded-lg p-4 bg-blue-50 ring-1 ring-blue-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="text-sm text-gray-700">
              <p>
                Olá, <strong>{order.cliente.nome}</strong> — recebemos seu pedido para o plano{" "}
                <strong>{order.produto.plano}</strong> da <strong>{order.produto.seguradora}</strong>.
              </p>
              <p className="mt-1">
                Viagem de <strong>{order.viagem.embarque}</strong> a <strong>{order.viagem.desembarque}</strong> —{" "}
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border p-4">
            ...
            <dl className="mt-2 text-sm text-gray-700 space-y-1.5">
              ...
              <div className="flex justify-between">
                <dt>DMH</dt>
                <dd className="font-medium">USD {order.produto.coberturaMedicaUSD.toLocaleString()}</dd>
              </div>
              <div className="flex justify-between">
                <dt>Bagagem</dt>
                <dd className="font-medium">USD {order.produto.bagagemUSD.toLocaleString()}</dd>
              </div>
              <div className="flex justify-between">
                <dt>Cancelamento</dt>
                <dd className="font-medium">USD {order.produto.cancelamentoUSD.toLocaleString()}</dd>
              </div>
              <div className="flex justify-between">
                <dt>COVID-19</dt>
                <dd className="font-medium">{order.produto.covid ? "Incluído" : "Não incluso"}</dd>
              </div>
            </dl>
          </div>
            ...
            <div className="mt-2 text-sm text-gray-700 space-y-1.5">
              <p className="flex items-center justify-between">
                ...
                <span className="font-medium">
                  ...
                  {order.pagamento.parcelas ? ` • ${order.pagamento.parcelas}x` : ""}
                </span>
              </p>
              <div className="border-t my-2" />
              <p className="flex items-center justify-between">
                <span>Subtotal</span> <span className="font-medium"><Money value={order.preco.subtotal} /></span>
              </p>
              <p className="flex items-center justify-between">
                <span>Taxas</span> <span className="font-medium"><Money value={order.preco.taxas} /></span>
              </p>
              <p className="flex items-center justify-between">
                <span>Desconto</span> <span className="font-medium text-blue-700">- <Money value={order.preco.desconto} /></span>
              </p>
              <div className="border-t my-2" />
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border p-4">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Em caso de necessidade, entre em contato com a central 24h informando o número do pedido{" "}
              <strong>{order.id}</strong>. O atendimento indica o prestador mais próximo.
            </p>
          </div>
          <div className="rounded-lg border p-4">
            ...
            <div className="mt-2 text-sm text-gray-700 space-y-1.5">
              ...
              <p className="flex items-center gap-2">
                <Phone className="h-4 w-4 text-gray-500" /> {order.cliente.telefone}
              </p>
              <p className="flex items-center gap-2">
                <ShieldCheck className="h-4 w-4 text-gray-500" /> Cobertura válida durante todo o período da viagem.
              </p>
            </div>
          </div>
        </div>
