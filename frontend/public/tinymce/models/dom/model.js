...
(() => {
	...
	const detectBrowser$1 = (browsers, userAgentData) => {
		return findMap(userAgentData.brands, (uaBrand) => {
			...
			return find$1(browsers, (browser) => {
				...
				current: info.name,
				version: Version.nu(Number.parseInt(uaBrand.version, 10), 0),
			}));
		});
	};
      ...
      */
		const get = (element) => {
			...
			if (r <= 0 || r === null) {
				...
				return Number.parseFloat(css) || 0;
			}
			return r;
		};
		...
		const aggregate = (element, properties) =>
			foldl(
				...
				(acc, property) => {
					...
					const value = val === undefined ? 0 : Number.parseInt(val, 10);
	...
	const getCssValue = (element, property) =>
		Number.parseInt(get$9(element, property), 10);
	...
	const getLockedColumnsFromGrid = (grid) => {
		...
		const lockedArr = mapToArray(locked, (_val, key) =>
	...
	const widthLookup = (table, getter) => () => {
		...
		if (inBody(table)) {
			...
			return Number.parseFloat(getRaw$2(table, "width").getOr("0"));
		}
	};
	...
	const percentageSize = (table) => {
		const getFloatWidth = widthLookup(table, (elem) =>
			Number.parseFloat(getPercentTableWidth(elem)),
		);
		const getWidth = widthLookup(table, get$7);
    ...
    */
			...
			mutation.get().each((target) => {
				hoverTable.each((table) => {
					getResizer(target, "data-row").each((row) => {
						...
						events.trigger.adjustHeight(table, delta, Number.parseInt(row, 10));
					});
	...
	const unmergable = (selectedCells) => {
		const hasSpan = (elem, type) =>
			getOpt(elem, type).exists((span) => Number.parseInt(span, 10) > 1);
	...
	const TableResizeHandler = (editor) => {
		...
		const afterCornerResize = (table, origin, width, height) => {
			...
			if (width !== startW && startRawW !== "") {
				...
				const percentW = Number.parseFloat(startRawW.replace("%", ""));
